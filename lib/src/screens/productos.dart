...
class _PantallaProductosState extends State<PantallaProductos> {
  ...
  Future<void> fetchProductos() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        productos = data['data']; // Accede a la clave 'data' en la respuesta
      });
    } else {
    ...
    await showDialog(
      ...
      builder: (BuildContext context) {
        return EditarProductoModal(
          producto: producto,
          onEditComplete: () {
            fetchProductos(); // Refresca la lista de productos después de editar
          },
        );
      },
    );
            ...
            trailing: Row(
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () => eliminarProducto(producto['_id']),
                ),
              ],
            ),
...
class _EditarProductoModalState extends State<EditarProductoModal> {
  ...
  Future<void> _fetchCategorias() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        _categorias = data['data']; // Accede a la clave 'data' en la respuesta
      });
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _editarProducto() async {
    ...
    final response = await http.put(
      ...
      body: jsonEncode({
        ...
        'stockDisponible': int.parse(_stockDisponible),
      }),
    );
